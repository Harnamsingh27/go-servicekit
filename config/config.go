...
// Load calls Validate after all sources have been merged.
...
// WithYAMLFile tells the loader to read configuration from a YAML file.
func WithYAMLFile(path string) Option {
	return func(o *options) { o.yamlFile = path }
}

// WithEnvFile tells the loader to read KEY=VALUE pairs from a file.
// Lines starting with # and blank lines are ignored.
func WithEnvFile(path string) Option {
	return func(o *options) { o.envFile = path }
}

// WithOverrides injects caller-supplied values at the highest precedence.
...
// Load builds a value of type T by merging all configured sources in
// precedence order (lowest to highest):
//
//  1. Struct "default" tags
//  2. YAML file (WithYAMLFile)
//  3. .env file (WithEnvFile)
//  4. System environment variables
//  5. Explicit overrides (WithOverrides)
func Load[T any](opts ...Option) (T, error) {
	...
	if o.envFile != "" {
		fileVars, err := parseEnvFile(o.envFile)
		if err != nil {
			return cfg, fmt.Errorf("config: env file: %w", err)
		}
		if err := applyMap(rv, fileVars); err != nil {
			return cfg, fmt.Errorf("config: env file vars: %w", err)
		}
	}

	if err := applySystemEnv(rv); err != nil {
		return cfg, fmt.Errorf("config: env vars: %w", err)
	}

	if len(o.overrides) > 0 {
		if err := applyMap(rv, o.overrides); err != nil {
...
func applyMap(rv reflect.Value, m map[string]string) error {
	rt := rv.Type()
	for i := range rt.NumField() {
		field := rt.Field(i)
		fv := rv.Field(i)
...
func applySystemEnv(rv reflect.Value) error {
	rt := rv.Type()
	for i := range rt.NumField() {
		field := rt.Field(i)
		fv := rv.Field(i)
		...
		if field.Type.Kind() == reflect.Struct && field.Type != reflect.TypeOf(time.Time{}) {
			if err := applySystemEnv(fv); err != nil {
				return err
			}
			continue
		}
...
func checkRequired(rv reflect.Value) error {
	...
	for i := range rt.NumField() {
		...
		if fv.IsZero() {
			hint := field.Name
			if k := field.Tag.Get("env"); k != "" {
				hint = k
			} else if k := field.Tag.Get("yaml"); k != "" {
				hint = k
			}
	...
	case reflect.Slice:
		if fv.Type().Elem().Kind() == reflect.String {
			...
			for idx, p := range parts {
				sl.Index(idx).SetString(strings.TrimSpace(p))
			}
			fv.Set(sl)
		}
	default:
